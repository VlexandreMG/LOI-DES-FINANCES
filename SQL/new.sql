...
('PIP externe','2024','3265.6'),
('PIP interne','2025','2368.4'),
('PIP externe','2025','5897.4');
...
CREATE OR REPLACE VIEW depenses_rubrique_24
AS(
    SELECT 
    cat.nom,prev.annee,prev.montant
    FROM categories_depenses AS cat JOIN previsions_depenses AS prev
    ON cat.id=prev.categorie_id WHERE prev.annee='2024'
);
CREATE OR REPLACE VIEW depenses_rubrique_25
AS(
    SELECT 
    cat.nom,prev.annee,prev.montant
    FROM categories_depenses AS cat JOIN previsions_depenses AS prev
    ON cat.id=prev.categorie_id WHERE prev.annee='2025'
);
CREATE OR REPLACE VIEW depenses_rubrique 
AS(
    SELECT 
...
CREATE OR REPLACE VIEW DSD24 
AS(
    SELECT * FROM depenses_soldes_detail WHERE annee='2024'
);
CREATE OR REPLACE VIEW DSD25 
AS(
    SELECT * FROM depenses_soldes_detail WHERE annee='2025'
);
CREATE OR REPLACE VIEW depenses_fonctionnement_2024
AS(
    SELECT * FROM  depenses_fonctionnement WHERE annee='2024'
);
CREATE OR REPLACE VIEW depenses_fonctionnement_2025
AS(
    SELECT * FROM  depenses_fonctionnement WHERE annee='2025'
);

CREATE OR REPLACE VIEW depenses_ivestissement_2024 
AS(
    SELECT * FROM depenses_ivestissement WHERE annee='2024'
);
CREATE OR REPLACE VIEW depenses_ivestissement_2025 
AS(
    SELECT * FROM depenses_ivestissement WHERE annee='2025'
);
CREATE OR REPLACE VIEW budget_admin
AS(
    SELECT min.nom, bud.annee, bud.montant
    FROM 
    ministeres as min 
    JOIN budget_ministeres as bud 
    ON min.id = bud.ministere_id
);
CREATE OR REPLACE VIEW budget_admin_2024 AS(
    SELECT * FROM budget_admin WHERE annee='2024'
);
CREATE OR REPLACE VIEW budget_admin_2025 AS(
    SELECT * FROM budget_admin WHERE annee='2025'
);
CREATE OR REPLACE VIEW rattache 
AS(
    SELECT 
    avant.nom,avant.montant AS LFR2024,apres.montant AS LF2025
    FROM budget_admin_2024 as avant 
    JOIN budget_admin_2025 as apres ON
    avant.nom=apres.nom
);
CREATE OR REPLACE VIEW dep_fonctionnement
AS(
    SELECT D24.type_depense, D24.montant AS DS24 ,D25.montant AS DS25, D25.montant-D24.montant AS ecart
     FROM 
    depenses_fonctionnement_2024 AS D24
